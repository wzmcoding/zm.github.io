...
## 请求缓存
`createCacheRequest`是一个高阶函数，用于为异步请求创建一个临时的缓存层。它可以在指定的时间内缓存相同的请求结果，从而避免重复请求，提高应用性能。
...
1. `fn`: 需要被缓存的异步函数
2. `ms`: 缓存持续时间(毫秒)，默认为0
3. `generatorKey`: 自定义的缓存键生成函数，默认使用 JSON.stringify

### 工作原理
1. 创建一个闭包内的`map`对象用于存储缓存
2. 返回一个新函数，接收原函数的所有参数
3. 根据参数生成缓存键
4. 如果缓存中存在结果，直接返回缓存的Promise
5. 如果缓存不存在，执行原函数并缓存结果
6. 在指定时间后自动清除缓存
